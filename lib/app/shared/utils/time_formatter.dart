...
String formatTime(int timestamp) {
  ...
  return !result.startsWith("A") ? result + ' atrás' : result;
}
...
String countSeconds(int difference) {
  ...
  return count > 1 ? count.toString() + ' segundos' : 'Agora';
}
...
String countDays(int difference) {
  ...
  return count.toString() + (count > 1 ? ' dias' : ' dia');
}
...
String countMonths(int difference) {
  ...
  return count.toString() + (count > 1 ? ' meses' : ' mês');
}
...
String countYears(int difference) {
  ...
  return count.toString() + (count > 1 ? ' anos' : ' ano');
}
